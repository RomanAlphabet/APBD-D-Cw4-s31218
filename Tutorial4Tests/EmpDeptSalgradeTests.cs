...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        var result = emps.Where(e => depts.Where(d=>d.Loc == "CHICAGO").Select(d=> d.DeptNo).Contains(e.DeptNo)).ToList(); 
    ...
    public void ShouldSelectNamesAndSalaries()
    {
        ...
        var result = emps.Select(e=> new {e.EName, e.Sal}).ToList(); 
         ...
         Assert.All(result, v =>
         {
             Assert.False(string.IsNullOrWhiteSpace(v.EName));
             Assert.True(v.Sal > 0);
         });
    }
    ...
    public void ShouldJoinEmployeesWithDepartments()
    {
        ...
        var result = emps
            .Join(depts,e => e.DeptNo,d => d.DeptNo,(emp, dept) => new {emp.EName, dept.DName}).ToList(); 
    ...
    public void ShouldReturnEmployeesWithCommission()
    {
        var emps = Database.GetEmps();
         ...
         var result = emps.Where(e => e.Comm is not null).Select(e=> new{e.EName, e.Comm}); 
    ...
    public void ShouldCalculateAverageSalaryPerDept()
    {
        var emps = Database.GetEmps();
         ...
         var result = emps.GroupBy(e=>e.DeptNo).Select(e=> new { DeptNo = e.Key, AvgSal = e.Average(e=>e.Sal) }); 
